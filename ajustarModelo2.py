...
class PrintCallback(tf.keras.callbacks.Callback):
    ...
    def print_loss_acc(self, logs, last=False, time=None):
        ...
        rmse = sum(self.rmse[-self.losses_avg_no:])/self.print_epoch
        if last:
...
def main():
    ...
    for pred_date in ['24H']:#, '48H', '72H']:
        for optimizers in ['Adam', 'SGD','RMSprop']:
            for train_eval in ['val_root_mean_squared_error', 'val_loss']:
                ...
                if train_model:
                    print("##########################################################")
                    print("Training model with {} optimizer and {} train_eval".format(optimizers, train_eval))
                    print("##########################################################")
                    print("Start training the model...")
                    checkpoint = keras.callbacks.ModelCheckpoint(checkpoint_filepath, monitor=train_eval, verbose=0, save_best_only=True, mode='min')
                    tensorboard = tf.keras.callbacks.TensorBoard(log_dir='LOGS/train/'+normalize+'_'+pred_date+'_'+str(LAYER1_NEURONS)+'_'+str(LAYER2_NEURONS)+'_'+str(NUM_EPOCHS)+'_'+optimizers+'_'+train_eval),
                    model.fit(X_train,y_train,epochs=NUM_EPOCHS, verbose=0, use_multiprocessing=True,
                        callbacks=[checkpoint,tensorboard,PrintCallback(PRINT_EPOCH, LOSSES_AVG_NO),],
